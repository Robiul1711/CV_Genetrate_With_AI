...
import {  Application, Assistant, CoachIcon, CreateIcon, Multilingual, Optimizer, ResumeUpdate, UpgradeIcon } from '../AllIcons/HomeIcons'
...
const OurFeatures = () => {
  return (
    ...
    <div className='flex items-center justify-center'>
        <p className="flex items-center gap-2 text-3xl font-medium"><UpgradeIcon />Our Features</p>
    </div>
    <div className='grid grid-cols-2 gap-6 mt-16'>
      {data.map((item) => (
        <div key={item.id} className='bg-[#0E0E10] rounded-[32px] p-10 '>
        <span className='border  rounded-3xl    p-6 inline-block'>{item.icons}</span>
        <h1 className='text-3xl font-medium pt-20'>{item.title}</h1>
        <p className='text-2xl text-[#9B9B9B] pt-2'>{item.desc}</p>
      </div>
      ))}
    </div>
<div className='mt-8 flex items-center justify-center text-center flex-col'>
   <div  className='bg-[#0E0E10] rounded-[32px] p-10 w-full '>
        <span className='border  rounded-3xl    p-6 inline-block'><Assistant/></span>
        <h1 className='text-3xl font-medium pt-20'>Interview Coach</h1>
        <p className='text-2xl text-[#9B9B9B] pt-2 max-w-[840px]  mx-auto'>Practice with AI-powered mock interviews. Master your interview skills with personalized AI feedback. Get ready for your dream job with realistic practice sessions.</p>
